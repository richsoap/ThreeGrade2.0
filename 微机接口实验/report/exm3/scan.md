...
* 其他可扩展功能
## 思考题
1. 段码表的八位数字中的每一位都对应着相应位置数码管的高低电平，进而可以决定哪些数码管亮哪些数码管暗。而位码中的每一位都对应着数码管是否被选通，也就决定了段码会在哪一个数码管显示。
2. 示例程序并不能判断多个键被按下的情况，只能判断出所有被选中的行和列，也就只能给出被选中的范围，但并不一定能知道具体是哪些键被按下。需要在知道范围之后再分别指定不同的高低电平将其区分开来。
## 源码
### 数字显示
```
#include <C8051F020.h>
